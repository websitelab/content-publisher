# Content Publisher — Build Prompt

## What This Is

A standalone automated content publisher for **Website Lab**.
...
│        - Post description embedded in PR body            │
...
│  5. Facebook auto-post (if site has facebook config)     │
│     → Title + description + article link                 │
│     → OG preview card auto-generated by Facebook         │
│     → Failure alert email to hello@websitelab.biz        │
└──────────────────────────────────────────────────────────┘
```
...
| Social sharing     | Facebook Graph API v25.0 (Pages)           |
...
4. **PR** — A pull request is created on the target site's repo (description embedded in PR body)
...
7. **Action** — Reviewer decides:
   - **Approve** → merges PR → Vercel deploys → Facebook auto-post → post is live
   - **Deny** → PR is closed, post is discarded
   - **Edit** → reviewer comments with feedback, post can be regenerated
8. **Auto-publish** — If no action after 24hr, PR auto-merges → Vercel deploys → Facebook auto-post
...
[
  {
    ...
    "facebook": {
      ...
      "tokenEnvVar": "FB_PAGE_TOKEN_MSJC",
...
| `facebook.articlePath` | URL path prefix for articles (e.g., `/articles`) |
...
When a blog post PR is merged (manually or after 24hr auto-publish), the system
automatically posts to the site's Facebook Page if a `facebook` config exists in
`sites.json`.

### How It Works

1. PR is created with the post description embedded as an HTML comment in the body:
   `<!-- blog-description:... -->`
2. On merge, `postToFacebook()` extracts the description, derives the slug from
   the PR title, and constructs the article URL
3. Posts to the Facebook Graph API with the title, description, and article link
4. Facebook auto-generates an OG preview card (image, title, description) from
   the page's meta tags

### Failure Handling

- Facebook posting is isolated in its own try/catch — it never breaks the merge
  flow or the user-facing approval response
- On failure, an alert email is sent to `hello@websitelab.biz` via Resend with
  the site, post title, and error details
- Sites without a `facebook` config are silently skipped

### Setup for a New Site

1. Create or use an existing Meta Developer App at developers.facebook.com
2. Add the "Manage everything on your Page" use case
3. In the Graph API Explorer, add `pages_manage_posts` permission
4. Generate a User Access Token → exchange for long-lived token →
   get Page Access Token (permanent for Pages)
5. Add the `facebook` block to the site's entry in `sites.json`
6. Store the Page Access Token as:
   - GitHub Secret (for auto-publish.js)
   - Vercel env var (for approve.js)

### Token Lifespan

Page Access Tokens derived from long-lived User Access Tokens are permanent —
they do not expire. No refresh logic needed.
...
8. Add labels: `content-publisher`, `auto-generated`
    ...
    steps:
      ...
      - run: node scripts/publish.js
        env:
          ...
          REVIEW_API_URL: https://content-publisher-iota.vercel.app
```
...
Set on the content-publisher repo: `gh secret set <NAME> --repo websitelab/content-publisher`
...
| `FB_PAGE_TOKEN_MSJC`| Facebook Page Access Token for MI Spine & Joint|
| `FB_PAGE_TOKEN_CSSC`| Facebook Page Access Token for Comp. Spine & Sports |

The Facebook tokens must also be set as **Vercel environment variables** on the
content-publisher project (for `api/approve.js`).
...
├── .github/
│   └── workflows/
│       ├── publish.yml          # Weekly content publisher (Tues 10am EDT)
...
│   ├── auto-publish.js          # Merge stale PRs (24hr+) + Facebook auto-post
│   ├── facebook.js              # Facebook Graph API auto-poster + failure alerts
...
- **Facebook auto-post** — posts to Facebook Page on merge (approve or auto-publish)
- **Isolated social posting** — FB failures never break the merge flow, alert email sent on failure
...
- **Facebook post failure** — logged, alert email sent to hello@websitelab.biz, merge unaffected
- **Facebook token missing** — silently skipped with log message
- **No facebook config** — site silently skipped (e.g., websitelab.biz)
